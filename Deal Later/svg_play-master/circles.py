...
class Circle:
    ...
    def draw_circle(self):
        ...
        for _ in range(self.rings-1):
            ...
            svg.write(f'<circle cx="{self.cx}" cy="{self.cy}" r="{rad}" stroke-width="{sw}" class="c{self.icolour}"/>')            
        sw = random.choice([1,1,1,2,2,3])
        svg.write(f'<circle cx="{self.cx}" cy="{self.cy}" r="{self.r}" stroke-width="{sw}" class="c{self.icolour}"/>')
...
class Circles:
    ...
    def svg_styles(self):
        ...
        svg.write('circle {stroke: black;}')
        for i, c in enumerate(self.colours):
            svg.write(f'.c{i} {{fill: {c}; }}')
